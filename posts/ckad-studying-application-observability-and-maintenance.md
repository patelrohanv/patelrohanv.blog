...
* access data with Metrics API
  ...
  1. install Metrics Server (component that gatheres the materics data for the Metrics API to return) -> <https://github.com/kubernetes-sigs/metrics-server> -> `kubectl apply -f https://github.com/kubernetes-sigs/metrics-server/releases/latest/download/components.yaml`
  2. use `kubectl top` to view metrics data once the Metrics Server gathered it (takes a few moments to gather data)
...
* check object configuration - gather more information on the relevant object(s)
  ...
  * use `kubectl describe` or view the full object manifest
* check logs - container level and potentially cluser level to get more insight into what's going on
